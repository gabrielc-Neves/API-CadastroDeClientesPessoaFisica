package API.cadastroDeClientesPessoaFisica.service;
...
class ClienteServiceTest {
    ...
    void deveListarTodosClientes() {
        ...
        when(clienteRepository.findAll(any(org.springframework.data.domain.Pageable.class))).thenReturn(page);

        Page<ClienteResponseDTO> result = clienteService.listarTodos(0, 10);

        assertEquals(1, result.getContent().size());
        assertEquals(cliente.getNome(), result.getContent().get(0).getNome());
    }
